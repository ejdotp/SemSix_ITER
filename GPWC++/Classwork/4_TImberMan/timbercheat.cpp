...
int main()
{
    ...
    bool cheatActive = false;     // A flag for cheat mode
    bool cheatKeyPressed = false; // To track if space was pressed during cheat mode
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            if (event.type == Event::KeyReleased && !paused)
            {
                ...
                // Handle space key release during cheat mode
                if (cheatActive && event.key.code == Keyboard::Space)
                {
        ...
        if (cheatActive && !paused) // If cheat is active and game is running
        {
            // Check for space key press to trigger cheat chop
            if (Keyboard::isKeyPressed(Keyboard::Space) && !cheatKeyPressed)
            {
                cheatKeyPressed = true;

                // Check for special scenarios first
                if (branchPositions[5] == side::LEFT && branchPositions[4] == side::RIGHT)
                {
                    playerSide = side::LEFT;
                }
                else if (branchPositions[5] == side::RIGHT && branchPositions[4] == side::LEFT)
                {
                    playerSide = side::RIGHT;
                }
                else
                {
                    // Determine chop direction based on branch positions
                    // Check bottom branch first (preferred position)
                    if (branchPositions[5] == side::LEFT)
                    {
                        playerSide = side::RIGHT; // Chop opposite to bottom branch
                    }
                    else if (branchPositions[5] == side::RIGHT)
                    {
                        playerSide = side::LEFT; // Chop opposite to bottom branch
                    }
                    else // If no bottom branch, check second from bottom
                    {
                        if (branchPositions[4] == side::LEFT)
                        {
                            playerSide = side::RIGHT;
                        }
                        else if (branchPositions[4] == side::RIGHT)
                        {
                            playerSide = side::LEFT;
                        }
                        else // If no branches at bottom, chop based on top branch
                        {
                            if (branchPositions[0] == side::LEFT)
                            {
                                playerSide = side::RIGHT;
                            }
                            else if (branchPositions[0] == side::RIGHT)
                            {
                                playerSide = side::LEFT;
                            }
                        }
        ...
        else if (acceptInput) // Normal gameplay controls
        {
