...
int main()
{
	...
	while (window.isOpen())
	{
		...
		// Activate cheat mode with spacebar
		...
		if (cheatActive) // If cheat is active, automatically make the player cut the tree
		{
			// Check branch positions
			if (branchPositions[0] == side::LEFT && branchPositions[1] == side::RIGHT)
			{
				// Cut left (same side as the first branch)
				playerSide = side::LEFT;
			}
			else if (branchPositions[0] == side::LEFT && branchPositions[1] == side::LEFT)
			{
				// Cut right (opposite side as the first branch)
				playerSide = side::RIGHT;
			}
			else if (branchPositions[0] == side::RIGHT && branchPositions[1] == side::LEFT)
			{
				// Cut right (opposite side as the first branch)
				playerSide = side::RIGHT;
			}
			else if (branchPositions[0] == side::RIGHT && branchPositions[1] == side::RIGHT)
			{
				// Cut left (same side as the first branch)
				playerSide = side::LEFT;
			}

			// Simulate cutting the tree
			...
			// Disable input for cheat action
			acceptInput = false;
		}
		else if (acceptInput) // make sure we are accepting input
		{
...
int main()
{
	...
	// Modify to include cheat input detection (Spacebar)
	bool cheatActive = false; // A flag for cheat mode
	...
	while (window.isOpen())
	{
		...
		{
			// Handle left and right key press for normal gameplay
			if (Keyboard::isKeyPressed(Keyboard::Left))
			{
				playerSide = side::LEFT;
				score++;
		...
		// The rest of your update and rendering code continues as it is...
